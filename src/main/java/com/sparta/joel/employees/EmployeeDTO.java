...
public class EmployeeDTO {
    ...
    public static final HashMap<Integer, Employee> employeesList = new HashMap<>();
    public static final LinkedBlockingQueue<Employee> employeeQueue = new LinkedBlockingQueue<>();
    ...
    public static void getData(boolean large){
        ...
        try(BufferedReader input = new BufferedReader(new FileReader(employees))){
            ...
            while(line != null){
                ...
                Employee check = employeesList.putIfAbsent(Integer.parseInt(attributes[0]),e);
                if(check == null){
                    employeeQueue.add(e);
                }
                line = input.readLine();
